...
function DashboardContent() {
  ...
  return (
    <AdminLayout
      ...
      headerActions={
        <>
          <button onClick={() => router.push('/admin/blog/new')} className="px-4 py-2 text-slate-600 text-sm font-medium hover:text-slate-900 transition-colors">
            Nuevo post
          </button>
        ...
        <div>
          ...
          <div className="bg-white rounded-xl shadow-sm overflow-hidden" style={{ border: '1px solid #CBD5E1' }}>
            <div className="p-4 flex items-center justify-between" style={{ background: '#E2E8F0', borderBottom: '1px solid #CBD5E1' }}>
              ...
              <Link href="/admin/blog" className="text-adm-primary text-[12px] font-medium hover:underline">Ver todo</Link>
            </div>
            {posts.length === 0 ? (
              ...
              <ul className="divide-y divide-slate-200">
                {posts.slice(0, 5).map(post => (
                  <li key={post.id}>
                    <Link href={`/admin/blog/${post.id}`} className="p-4 hover:bg-slate-50 transition-colors flex items-start gap-3">
