    ...
    <section className="px-[clamp(18px,4vw,56px)] py-[clamp(80px,10vw,140px)]">
      <div className="mx-auto max-w-[1240px]">
        <div data-reveal className="mb-[clamp(36px,4vw,56px)] flex flex-wrap items-end justify-between gap-6">
          <h2 className={`${H2} m-0 max-w-[16ch] text-primary`}>
            Todo listo para <Accent>vender</Accent>
          </h2>
          <p className={`${BODY} m-0 max-w-[36ch] text-[#7B8DB0]`}>
          ...
          {INCLUDES.map(it => (
            <li
              ...
              className={`flex min-h-[180px] flex-col justify-between gap-9 rounded-[22px] p-6 ${
                it.hot ? "bg-accent text-white sm:col-span-2" : "border border-white/[0.07] bg-[#0D1117] text-primary"
              }`}
            >
              ...
              <div>
                ...
                <div className={`text-sm leading-normal ${it.hot ? "text-white/80" : "text-[#7B8DB0]"}`}>{it.s}</div>
              </div>
            </li>
          ))}
        </ul>
