...
export default function AdminLayout({ children, title, headerActions }: AdminLayoutProps) {
  ...
  const breadcrumb = pathname.startsWith('/admin/blog/')
          ...
          {nav.map(({ to, label, icon }) => {
            ...
            return (
              <Link
                ...
                style={isActive ? { background: '#4361EE', color: '#ffffff' } : {}}
              >
                <span className="material-symbols-outlined" style={{ fontSize: 20 }}>{icon}</span>
                <span className="text-[13px] flex-1">{label}</span>
                {to === '/admin/leads' && unreadLeads > 0 && (
                  <span className="text-[10px] font-bold bg-[#EF4444] text-white rounded-full px-1.5 py-0.5 leading-none min-w-[18px] text-center">
                    {unreadLeads}
                  </span>
                )}
              </Link>
      ...
      <div className="ml-[260px] flex-1 flex flex-col min-h-screen">
        <header className="fixed top-0 right-0 left-[260px] h-16 bg-white/80 backdrop-blur-md border-b border-slate-200 z-40 flex items-center justify-between px-8">
          <div className="flex items-center gap-2">
            {breadcrumb.length > 1 ? (
              <>
                <Link href={breadcrumb[0].href!} className="text-slate-500 text-[14px] hover:text-adm-primary transition-colors">
                  {breadcrumb[0].label}
                </Link>
