      ...
      >
        ...
        <nav className="hidden md:flex items-center gap-8 lg:gap-12">
          {navItems.map(item => {
            ...
            if (item === 'Inicio') return <Link key={item} href="/" className={cls}>{item}</Link>
            if (item === 'Blog')   return <Link key={item} href="/blog" className={cls}>{item}</Link>
            return (
              <a
                key={item}
        ...
        <div className="md:hidden w-[calc(100%-2rem)] max-w-sm mt-2 bg-[#07090F] border border-white/[0.06] rounded-2xl overflow-hidden">
          <nav className="flex flex-col">
            {navItems.map(item => {
              ...
              if (item === 'Inicio') return <Link key={item} href="/" onClick={() => setOpen(false)} className={cls}>{item}</Link>
              if (item === 'Blog')   return <Link key={item} href="/blog" onClick={() => setOpen(false)} className={cls}>{item}</Link>
              return (
                <a
                  key={item}
                  href={`#${item.toLowerCase()}`}
