...
const DEFAULT_FAQS = [
  ...
  { q: '¿Cuánto tiempo tarda el desarrollo?', a: 'Las apps privadas pueden estar listas en 2-4 semanas. Las apps públicas para el App Store suelen tomar entre 6 y 12 semanas, incluyendo el proceso de revisión de Shopify.' },
  { q: '¿Publican las apps en el Shopify App Store?', a: 'Sí. Tenemos experiencia con todo el proceso de publicación: cumplimiento de requirements técnicos, listing, screenshots y proceso de review. Gestionamos todo.' },
  { q: '¿Solo trabajan con Shopify?', a: 'Nuestro foco es 100% Shopify. Esto nos permite ser mejores en lo que hacemos. No somos una agencia generalista — somos especialistas en el ecosistema Shopify.' },
  { q: '¿Qué pasa después del lanzamiento?', a: 'Ofrecemos planes de mantenimiento y soporte mensual. Monitoreamos la app, aplicamos actualizaciones de la API de Shopify y resolvemos bugs. El código fuente es tuyo.' },
  { q: '¿Cómo es el proceso de pago?', a: '40% al inicio del proyecto, 40% en el hito de entrega de la versión beta, y 20% restante al lanzamiento. Aceptamos transferencia bancaria y PayPal.' },
]
  ...
  return (
    ...
    >
      <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between gap-4 py-5 text-left group">
        ...
        <span className="text-[#7B8DB0] flex-shrink-0">{open ? <Minus size={16} /> : <Plus size={16} />}</span>
      </button>
...
export default function FAQ() {
  ...
  const heading    = c.heading    || 'Preguntas frecuentes'
