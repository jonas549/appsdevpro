...
const D = {
  ...
  apps: [
    ...
    },
    {
      ...
      desc:      "Descuentify es un motor de descuentos para Shopify que va más allá de las reglas nativas. Para merchants que necesitan descuentos por volumen escalonado, combos entre variantes específicas, bulk price editor masivo, campañas con condiciones combinadas y reglas Buy X Get Y avanzadas. El objetivo es subir el ticket promedio (AOV) sin canibalizar margen.",
  ...
  procH2:  "Un proceso de desarrollo claro, sin sorpresas y con entregas verificables",
  procSub: "Desde la primera reunión hasta el lanzamiento en producción, sabes exactamente qué esperar en cada etapa.",
  procSteps: [
...
function buildHtml(
  ...
  const servMain  = { title: get(cms, "services", "main_title", D.servItems[0].title), desc: get(cms, "services", "main_desc", D.servItems[0].desc) }
  ...
  const appItems  = D.apps.map((def, i) => ({
  ...
  const procSteps = D.procSteps.map((def, i) => ({
  ...
  const body = [
    `<header>`,
    `  <nav>`,
    `    <a href="${SITE_URL}">Apps Developers Pro</a>`,
    ...
    `  <section aria-label="El problema">`,
    `    <h2>${esc(probH2)}</h2>`,
    probDesc ? `    <p>${esc(probDesc)}</p>` : "",
    `  </section>`,

    `  <section aria-label="La solución">`,
    `    <h2>${esc(solH2)}</h2>`,
    ...solItems.map(it =>
      `    <article>\n      <h3>${esc(it.title)}</h3>\n      <p>${esc(it.desc)}</p>\n    </article>`
    ),
    ...
    `    <article>\n      <h3>${esc(servMain.title)}</h3>\n      <p>${esc(servMain.desc)}</p>\n    </article>`,
    ...servCards.map(it =>
      `    <article>\n      <h3>${esc(it.title)}</h3>\n      <p>${esc(it.desc)}</p>\n    </article>`
    ),
    `  </section>`,

    `  <section aria-label="Apps publicadas" id="apps">`,
    `    <h2>${esc(appsH2)}</h2>`,
    ...
    ...appItems.map(app => [
      ...
      `      <p>${esc(app.badge)}</p>`,
      `      <p>${esc(app.desc)}</p>`,
      `      <p>Tecnologías: ${esc(app.tags)}</p>`,
      app.store_url ? `      <a href="${esc(app.store_url)}">${esc(app.cta)}</a>` : `      <span>${esc(app.cta)}</span>`,
      `    </article>`,
    ].join("\n")),
    `  </section>`,
    ...
    `<footer>`,
    `  <p>Apps Developers Pro — <a href="mailto:contacto@appsdeveloperspro.com">contacto@appsdeveloperspro.com</a></p>`,
    `  <nav>`,
...
export default async function handler(_req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    console.warn("[ai-render] DB unavailable, using defaults:", (err as Error).message)
  }

  const seo   = cms["seo"] || {}
  const title = plain(seo.meta_title       || D.title)
  const desc  = plain(seo.meta_description || D.desc)
  const img   = plain(seo.og_image         || D.ogImage)

  const html = buildHtml(cms, title, desc, img)
